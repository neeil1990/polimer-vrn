...
foreach($arResult["CATEGORIES"] as $category_id => &$arCategory)
{
	...
	foreach($arCategory["ITEMS"] as $i => $arItem)
	{
		...
		if(isset($arItem["ITEM_ID"])){
			...
			$arResult["CATEGORIES"][$category_id]["ITEMS"][$i]['FORMAT_INT'] = (priceDiscount($arItem["ITEM_ID"])) ? CurrencyFormat(priceDiscount($arItem["ITEM_ID"]), 'RUB') : null;
	...
	if($arFilterSection['ID']){
		$arSectionIds = array();
		$res = CIBlockElement::GetList(Array(), $arFilterSection, false, false, ['ID', 'NAME', 'IBLOCK_SECTION_ID']);
		...
		if($arSectionIds){
			...
			while($ar_result = $db_list->GetNext())
			{
				$arSectionItems[] = [
					...
					'MODULE_ID' => $ar_result['MODULE_ID'],
					'PARAM1' => $ar_result['IBLOCK_TYPE_ID'],
					'PARAM2' => $ar_result['IBLOCK_ID'],
			...
			$arResult["CATEGORIES"][$category_id]["ITEMS"] = array_merge($arSectionItems, $arCategory["ITEMS"]);
		}
	}
}
