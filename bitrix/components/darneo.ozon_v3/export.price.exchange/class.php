...
use Darneo\Ozon\Export\Price\Manager as ExportPriceManager;
use Darneo\Ozon\Export\Table\PriceListTable;
use Darneo\Ozon\Export\Table\PriceLogTable;

class OzonExportPriceExchangeComponent extends CBitrixComponent
{
    ...
    private ExportPriceManager $manager;
    ...
    private function dataManager(): void
    {
        ...
        $this->manager = new ExportPriceManager($this->elementId);
        switch ($this->trigger) {
        ...
        $this->settings = PriceListTable::getById($this->elementId)->fetch();
    }
    ...
    private function setTemplateData(): void
    {
        ...
        $this->arResult['DATA_VUE'] = [
            'COUNT_HELPER' => Loc::getMessage('DARNEO_OZON_MODULE_PRICE_EXCHANGE_COUNT_HELPER_' . $this->trigger),
            'STATUS_HELPER' => Loc::getMessage('DARNEO_OZON_MODULE_PRICE_EXCHANGE_STATUS_HELPER_' . $this->trigger),
            'PAGE' => $this->page,
        ...
        $this->arResult['SIGNED_PARAMS'] = (new Signer())->sign(
            ...
            'darneo.ozon.export.price.exchange'
        );
    }
    ...
    private function initSend(): void
    {
        switch ($this->trigger) {
            ...
            case self::TRIGGER_MAIN:
                ...
                $answer = (new \Darneo\Ozon\Api\v1\Product())->importPrices($data);
                if ($answer['result']) {
