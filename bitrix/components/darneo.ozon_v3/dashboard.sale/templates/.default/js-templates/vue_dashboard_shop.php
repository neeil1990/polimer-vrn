<script>
BX.BitrixVue.component('ozon-dashboard-shop', {
    props: {
        data: {
            type: Object,
            required: true
    ...
    computed: {
        percent: function () {
            ...
            return Math.round((this.data.SUM_SHOP / this.data.SUM) * 100)
        },
        sumShop: function () {
            return Number(this.data.SUM_SHOP)
        },
        year: function () {
    ...
    methods: {
        ...
        getElementChart: function () {
            return document.getElementById('kt_apexcharts_shop')
        },
        ...
        animateNumber() {
            const element = document.getElementById('sum_count_shop')
            const startValue = 0
            const endValue = Number(this.sumShop)
            const duration = 1000
    ...
    template: `
        ...
        <div class='card-header border-0 py-5'>
            <h3 class='card-title align-items-start flex-column'>
                    <span class='card-label fw-bold fs-3 mb-1'>
                        {{ $Bitrix.Loc.getMessage('DARNEO_OZON_VUE_DASHBOARD_SHOP_TITLE') }}
                    </span>
        ...
        <div class='card-body d-flex flex-column'>
            <div class='flex-grow-1'>
                <div class='mixed-widget-4-chart' id='kt_apexcharts_shop' data-kt-chart-color='primary'
                     style='height: 200px'></div>
            </div>
            <div class='pt-5'>
                <div class='px-9 card-rounded w-100 bg-success'>
                    <div class='d-flex text-center flex-column text-white pt-8 pb-8'>
                        <span class='fw-bold fs-2x pt-1' id='sum_count_shop'></span>
                    </div>
                </div>
            </div>
        </div>
