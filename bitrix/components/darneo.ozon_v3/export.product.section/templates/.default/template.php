...
?>
<div id='vue-export-section'></div>
<script>
    $(function () {
        <?='BX.message(' . CUtil::PhpToJSObject(Loc::loadLanguageFile(__FILE__)) . ');'?>
        BX.Ozon.ExportSection.Vue.init({
            ajaxUrl: '<?=CUtil::JSEscape($arResult['PATH_TO_AJAX'])?>',
            signedParams: '<?=CUtil::JSEscape($arResult['SIGNED_PARAMS'])?>',
            section: <?=CUtil::PhpToJSObject($jsResult['DATA_VUE']['SECTION'])?>,
            filter: <?=CUtil::PhpToJSObject($jsResult['DATA_VUE']['FILTER'])?>,
