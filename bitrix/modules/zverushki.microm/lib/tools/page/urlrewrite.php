...
class UrlRewrite {
	...
	public function getRuleByUrl ($pageUrlTemplate, $requestPageUrl = false) {
		...
		$pageUrlTemplate = str_replace('#SITE_DIR#', SITE_DIR, $pageUrlTemplate);
		$pageUrlTemplate = str_replace('//', '/', $pageUrlTemplate);

		$pageUrlTemplateReg = preg_replace("'#[^#]+?#'", "([^/]+?)", $pageUrlTemplate);
		$requestPageUrl = $requestPageUrl
							? $requestPageUrl
							: $GLOBALS['APPLICATION']->GetCurUri('', false);

		list($requestPageUrl,) = explode('?', $requestPageUrl);
		$arValues = array();
		$arVariables = array();
