...
class Customibtab
{
	...
	function __construct () {
    	...
    	while ($arSite = $rsSites->Fetch()){
    		if(Option::get(self::$module_id, "microm_view_article", "N", $arSite["LID"]) == "Y")
    			$activeScheme = true;
    	}
		...
		Asset::getInstance()->addString('<script>
				...
				$(document).ready(function() {
					$("#microm-article").on("change", ".typeselect", function(){
						__val = $("option:selected", $(this)).val();
						console.log(__val);
						$("#microm-article .grouped_block.active").removeClass("active");
						$("#microm-article #hl_group_"+__val).addClass("active");
			...
			if(!empty($_post["subschema"]) && !empty($_post["subschema"]["values"])):
				...
				if(empty($errors)):
					...
					$result = MicromTable::getList(array(
							"select"	=> array("ID"),
							"filter"	=> array("SITE_ID" => "-", "CODE" => "article_ib_".$_obj->iblockId),
							"limit"		=> array()
						)
					);
					if($arr = $result->fetch())
	...
	public function getData(){
		...
		$result = MicromTable::getList(array(
				...
				"filter"	=> array("SITE_ID" => "-", "CODE" => "article_ib_".$this->iblockId),
				"limit"		=> array()
			)
		);
