...
foreach ($summaryValues as $itemIndex => $itemValue)
{
	...
	if ((string)$itemValue['DELIVERY_TYPE'] !== '')
	{
		$textParts[] = $lang['DELIVERY_TYPE_' . Data\TextString::toUpper($itemValue['DELIVERY_TYPE'])];
	}

	if ((string)$itemValue['NAME'] !== '')
	{
		$textParts[] = str_replace('#NAME#', $itemValue['NAME'], $lang['NAME']);
	}

	if ((string)$itemValue['ORDER_BEFORE'] !== '')
	{
	...
	$text = Data\TextString::ucfirst($text);
