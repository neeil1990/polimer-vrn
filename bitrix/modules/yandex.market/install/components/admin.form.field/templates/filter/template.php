...
$langStatic = [
	...
	'ADD_BUTTON' => isset($arParams['LANG_ADD_BUTTON']) ? $arParams['LANG_ADD_BUTTON'] : Loc::getMessage('YANDEX_MARKET_T_ADMIN_FIELD_FILTER_ADD_BUTTON'),
	'ADD_BUTTON_TOOLTIP' => isset($arParams['LANG_ADD_BUTTON']) ? $arParams['LANG_ADD_BUTTON_TOOLTIP'] : Loc::getMessage('YANDEX_MARKET_T_ADMIN_FIELD_FILTER_ADD_BUTTON_TOOLTIP'),
];
...
else // multiple
{
	...
	if ($arParams['FILTER_BASE_NAME'])
	{
		$collectionAttributes .= ' data-filter-base-name="' . $arParams['FILTER_BASE_NAME'] . '"';
	}
	...
	if ($arParams['NEED_LEFT_COUNT'] && $arParams['EXPORT_LEFT_MESSAGE'])
	{
	...
	<div class="<?= $arParams['HAS_VALUE'] ? '' : 'is--empty';  ?> spacing--1x1 <?= $arParams['CHILD'] ? $arParams['CHILD_CLASS_NAME'] : 'js-plugin'; ?> js-condition-manager" id="<?= $fieldId; ?>" data-plugin="Field.Filter.Collection" <?= $arParams['CHILD'] ? '' : 'data-base-name="' . $arParams['INPUT_NAME'] .'"';  ?> <?= $collectionAttributes; ?>>
		...
		foreach ($arParams['VALUE'] as $itemIndex => $itemValue)
		{
			...
			{
				?>
				<div class="b-block-sort <?= $arParams['ALLOW_NAME'] ? 'pin--bottom' : ''; ?> js-filter-collection__sort">
...
$managerData = [
	'iblockId' => $arParams['CONTEXT']['IBLOCK_ID'],
	'fields' => array_values($arResult['FIELD_ENUM']),
