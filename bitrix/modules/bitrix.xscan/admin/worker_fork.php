...
if (isset($_POST['hide-files'])) {
    ...
    if (!$all) {
        $filesId = Bitrix\Main\Web\Json::decode($_POST['hide-files']);
        $filter = ['@id' => $filesId];
    }

    $list = XScanResultTable::deleteList($filter);
...
if (isset($_REQUEST['ajax'])) {
    $break_point = isset($_REQUEST['break_point']) ? $_REQUEST['break_point'] : '';
    $clean = isset($_REQUEST['clean']) && $_REQUEST['clean'] == 'Y';
    \CBitrixXscanAjax::run($scaner, $nav, $start_path, $break_point, $clean);
    die();
}
?>
...
<script>
    ...
    function hit(file, action){
        BX.ajax.post('/bitrix/admin/bitrix.xscan_worker_fork.php',
            {
...
if ($action === 'showfile') {
    ...
    if (!$file || !file_exists($file)) {
        ...
        if ($res) {
            ...
            <div class="ui-alert ui-alert-icon-warning">
                <span class="ui-alert-message"><strong><?= GetMessage("BITRIX_XSCAN_SCORE") ?></strong> <?= htmlspecialcharsbx($scaner->getScore()) ?></span>
            </div>
        ...
        if (preg_match('/\.ph[_p][578]?$/', $file)) {
            echo strtolower(substr($file, -4)) !== '.ph_' ? CBitrixXscanFork::getIsolateButton($file) : CBitrixXscanFork::getUnIsolateButton($file);
        }
        ...
        echo '<a class="ui-btn ui-btn-primary ui-btn-sm" href="/bitrix/admin/bitrix.xscan_file_edit.php?path=' . urlencode($file) . '&full_src=Y&IFRAME=Y&back_url=' . urlencode($_SERVER['REQUEST_URI']) . '">' . GetMessage("BITRIX_XSCAN_EDIT_BTN") . '</a>';
...
if (in_array("xdebug", get_loaded_extensions())) {
    echo CBitrixXscanFork::ShowMsg(GetMessage("BITRIX_XSCAN_XDEBUG"), 'red');
}
...
?>
    ...
    <script>
            ...
            BX.ajax.post('/bitrix/admin/bitrix.xscan_worker_fork.php',
                {
                    'sessid': BX.bitrix_sessid(),
                    'ajax': true,
                    'hide-files': JSON.stringify(selectedIds),
        ...
        function go(clean = 'N', progress = '', total = '', break_point = '') {
            BX.ajax({
                url: '/bitrix/admin/bitrix.xscan_worker_fork.php',
                method: 'POST',
                data: {
                    'sessid': BX.bitrix_sessid(),
                    'action': 'scan',
