...
$sbermmSettings = $sbermm->getDetailSettings();
...
$arTypesConfig = array(
	'none' => array(
		'picture', 'offers-picture',
        'vendor', 'vendorCode', 'model', 'sales_notes',
        // sbermm
        'barcode', 'order-before', 'days',
		//'manufacturer_warranty', 'country_of_origin', 'adult'
	),
	/* 'vendor.model' => array(
...
while ($vatRow = $vatIterator->fetch()) {
    $vatValue = intval($vatRow['RATE']).'%';
    $vatRates[$vatValue] = $vatValue;
}
...
<?
	...
	else
	{
		...
		{
            $showOnlyProductsProps = ($key == 'picture');
            $showOnlyOffersProps = ($key == 'offers-picture');

			?><select style="min-width:370px;" name="XML_DATA[<? echo htmlspecialcharsbx($group)?>][<? echo htmlspecialcharsbx($key)?>]">
			<option value=""<? echo ($value == "" ? ' selected' : ''); ?>><?=GetMessage('YANDEX_SKIP_PROP')?></option>
			<?
            if (!$showOnlyOffersProps)
            {
			...
			if (!empty($IBLOCK['OFFERS_PROPERTY']) && !$showOnlyProductsProps)
			{
...
****************************************************************************/
        ...
        <script src="/bitrix/js/wbs24.sbermmexport/stringTemplate.js?<?=time()?>"></script>
        <style>
            .input_template {
                min-width: 364px;
            }
            ...
            <td colspan="2">
                <div style="padding: 10px;">
                    <table width="90%" class="inner" style="text-align: center;">
                        <tr>
                            <td align="right"><?=GetMessage('WBS24_SBERMMEXPORT_CUSTOM_PRUDUCT_NAME'); ?></td>
                            <td style="white-space: nowrap;">
                                <?=$sbermmSettings->getNameInput("COMMON_FIELDS[PRODUCT_NAME]", [
                                    'PRODUCTS' => $arIBlock['PROPERTY'],
                                ], $commonFields['PRODUCT_NAME']);?>
                                &nbsp;<small>(name)</small>
                            </td>
                        </tr>
                        <tr>
                            <td align="right"><?=GetMessage('WBS24_SBERMMEXPORT_CUSTOM_OFFER_NAME'); ?></td>
                            <td style="white-space: nowrap;">
                                <?=$sbermmSettings->getNameInput("COMMON_FIELDS[OFFER_NAME]", [
                                    'PRODUCTS' => $arIBlock['PROPERTY'],
                                    'OFFERS' => $arIBlock['OFFERS_PROPERTY'],
                                ], $commonFields['OFFER_NAME']);?>
                                &nbsp;<small>(name)</small>
                            </td>
                        </tr>
                        <tr>
                            <td align="right"><?=GetMessage('WBS24_SBERMMEXPORT_CUSTOM_PRUDUCT_URL'); ?></td>
                            <td style="white-space: nowrap;">
                                <?=$sbermmSettings->getSelect("COMMON_FIELDS[PRODUCT_URL]", $arIBlock['PROPERTY'], $commonFields['PRODUCT_URL']);?>
                                &nbsp;<small>(url)</small>
                            </td>
                        </tr>
                        <tr>
                            <td align="right"><?=GetMessage('WBS24_SBERMMEXPORT_CUSTOM_OFFER_URL'); ?></td>
                            <td style="white-space: nowrap;">
                                <?=$sbermmSettings->getSelect("COMMON_FIELDS[OFFER_URL]", $arIBlock['OFFERS_PROPERTY'], $commonFields['OFFER_URL']);?>
                                &nbsp;<small>(url)</small>
                            </td>
                        </tr>
                        <tr>
                            <td colspan="2" style="text-align: center;">
                                <div class="adm-info-message-wrap">
                                    <div class="adm-info-message"><?=GetMessage('WBS24_SBERMMEXPORT_URL_NOTE'); ?></div>
                                </div>
                            </td>
                        </tr>
                        <tr>
                            <td align="right"><?=GetMessage('BX_CATALOG_EXPORT_YANDEX_PICTURE'); ?></td>
                            <td style="white-space: nowrap;">
                                <select style="min-width:370px;" name="COMMON_FIELDS[PICTURE]">
                                    <option value="AUTO"<?=($commonFields['PICTURE'] == 'AUTO' ? ' selected' : '');?>><?=GetMessage('BX_CATALOG_EXPORT_YANDEX_PICTURE_AUTO'); ?></option>
                                    <option value="PREVIEW_PICTURE"<?=($commonFields['PICTURE'] == 'PREVIEW_PICTURE' ? ' selected' : '');?>><?=GetMessage('BX_CATALOG_EXPORT_YANDEX_PICTURE_PREVIEW_PICTURE'); ?></option>
                                    <option value="DETAIL_PICTURE"<?=($commonFields['PICTURE'] == 'DETAIL_PICTURE' ? ' selected' : '');?>><?=GetMessage('BX_CATALOG_EXPORT_YANDEX_PICTURE_DETAIL_PICTURE'); ?></option>
                                </select>
                                <small>(picture)</small>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td style="white-space: nowrap;">
                                <select style="min-width:370px;" name="COMMON_FIELDS[DESCRIPTION]">
...
<?
			foreach ($arConfig as $prop):
                if ($prop == 'order-before' || $prop == 'days') continue;
                $propCode = $prop;
                if ($propCode == 'offers-picture') $propCode = 'picture';
?>
						<tr>
						<td align="right"><?=htmlspecialcharsbx(GetMessage('YANDEX_PROP_'.$prop))?>: </td>
						<td style="white-space: nowrap;"><?__yand_show_selector($key, $prop, $arIBlock, (isset($arTypeValues[$key][$prop]) ? $arTypeValues[$key][$prop] : ''))?>&nbsp;<small>(<?=htmlspecialcharsbx($propCode)?>)</small></td>
						</tr>
<?
			endforeach;
?>
...
function __addYP()
{
	...
	oCell.innerHTML = strContent;
        ...
        <tr class="heading">
            <td colspan="2"><?=GetMessage('YANDEX_PROPS_SHIPMENT')?></td>
        </tr>
        <tr>
            <td colspan="2">
<?
        $arShipmentProps = $arIBlock;
        $arShipmentProps['OFFERS_PROPERTY'] = [];
        foreach ($arTypesConfig as $key => $arConfig):
?>
                <div id="config_<?=htmlspecialcharsbx($key)?>" style="padding: 10px; display: <? echo ($type == $key ? 'block' : 'none'); ?>;">
                    <table width="90%" class="inner" style="text-align: center;">
...
?>
                        <tr>
                        <td align="right"><?=htmlspecialcharsbx(GetMessage('YANDEX_PROP_'.$prop))?>: </td>
                        <td style="white-space: nowrap;"><?__yand_show_selector($key, $prop, $arShipmentProps, (isset($arTypeValues[$key][$prop]) ? $arTypeValues[$key][$prop] : ''))?>&nbsp;<small>(<?=htmlspecialcharsbx($prop)?>)</small></td>
                        </tr>
<?
            endforeach;
?>
        ...
        <tr>
            <td colspan="2">
                <div class="adm-info-message">
                    <span><?=GetMessage("BX_CATALOG_EXPORT_SHIPMENT_NOTE");?></span>
                </div>
            </td>
        </tr>
<?
...
?>
	...
	<tr>
		...
		<td><select class="select_price_type" name="PRICE">
...
?>
            <option value="-1"<?=(-1 == $PRICE ? ' selected' : '')?>><?=GetMessage('WBS24_SBERMMEXPORT_PRICE_FROM_PROPERTY');?></option>
		</select></td>
	</tr>
    <tr>
        <td><?=GetMessage('WBS24_SBERMMEXPORT_PRODUCT_PRICE_PROPERTIES');?>: </td>
        <td>
            <?=$sbermmSettings->getSelect(
                "COMMON_FIELDS[PRODUCT_PRICE_PROPERTY]",
                $arIBlock['PROPERTY'],
                $commonFields['PRODUCT_PRICE_PROPERTY'],
                [
                    'disabled' => ($PRICE != -1),
                    'default' => GetMessage("YANDEX_SKIP_PROP"),
                ]
            );?>
        </td>
    </tr>
    <tr>
        <td><?=GetMessage('WBS24_SBERMMEXPORT_OFFER_PRICE_PROPERTIES');?>: </td>
        <td>
            <?=$sbermmSettings->getSelect(
                "COMMON_FIELDS[OFFER_PRICE_PROPERTY]",
                $arIBlock['OFFERS_PROPERTY'],
                $commonFields['OFFER_PRICE_PROPERTY'],
                [
                    'disabled' => ($PRICE != -1),
                    'default' => GetMessage("YANDEX_SKIP_PROP"),
                ]
    ...
    <tr>
        <td><?=GetMessage('WBS24_SBERMMEXPORT_PACKAGING_RATIO_PRODUCT_PROPERTIES');?>: </td>
        <td>
            <?=$sbermmSettings->getSelect(
                "COMMON_FIELDS[PRODUCT_PACKAGE_RATIO_PROPERTY]",
                $arIBlock['PROPERTY'],
                $commonFields['PRODUCT_PACKAGE_RATIO_PROPERTY'],
                [
                    'default' => GetMessage("WBS24_SBERMMEXPORT_PACKAGING_RATIO_DEFAULT"),
                ]
            );?>
        </td>
    </tr>
    <tr>
        <td><?=GetMessage('WBS24_SBERMMEXPORT_PACKAGING_RATIO_OFFERS_PROPERTIES');?>: </td>
        <td>
            <?=$sbermmSettings->getSelect(
                "COMMON_FIELDS[OFFER_PACKAGE_RATIO_PROPERTY]",
                $arIBlock['OFFERS_PROPERTY'],
                $commonFields['OFFER_PACKAGE_RATIO_PROPERTY'],
                [
                    'default' => GetMessage("WBS24_SBERMMEXPORT_PACKAGING_RATIO_DEFAULT"),
                ]
            );?>
    ...
    <tr class="tr-ep-formula">
        ...
        <td width="60%" style="padding-top: 0;"><small><?=GetMessage("WBS24_SBERMMEXPORT_PACKAGING_RATIO_NOTE");?></small></td>
    </tr>

    <script>
    if (typeof selectPriceTypeHandler === 'undefined') {
        let selectPriceTypeHandler = function() {
            let priceSelect = document.querySelector('.select_price_type');
            let productPropSelect = document.querySelector('.select_common_fields_product_price_property');
            let offerPropSelect = document.querySelector('.select_common_fields_offer_price_property');

            if (priceSelect && productPropSelect && offerPropSelect) {
                priceSelect.addEventListener('change', () => {
                    let priceFromProps = (priceSelect.value == -1);
                    productPropSelect.disabled = !priceFromProps;
                    offerPropSelect.disabled = !priceFromProps;
                });
            }
        };
        selectPriceTypeHandler();
    }
    </script>
