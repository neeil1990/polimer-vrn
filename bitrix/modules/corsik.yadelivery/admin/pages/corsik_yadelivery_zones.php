...
if ($version >= 19 || !isset($version)) {
    ...
    $sTableID = "corsik_yadelivery_zones";
    $zonesList = new Corsik\YaDelivery\Table\ZonesTable();
...
//        "filterable" => "?"
    ...
    if ($arID = $lAdmin->GroupAction()) {
        foreach ($arID as $ID) {
            ...
            switch ($_REQUEST['action']) {
                case "delete":
                    $res = $zonesList->delete($ID);
                    break;
                case "activate":
                case "deactivate":
                    $arFields = ["ACTIVE" => ($_REQUEST['action'] == "activate" ? "Y" : "N")];
                    $res = $zonesList->update($ID, $arFields);
                    if (!$res->isSuccess()) {
    ...
    $dbResultList = $zonesList->getList($queryList);
    ...
    while ($dbrs = $rsIBlocks->NavNext(true, "f_")) {
        $row =& $lAdmin->AddRow($f_ID, $dbrs, 'corsik_yadelivery_zone_edit.php?ID=' . $f_ID . '&lang=' . LANGUAGE_ID, Loc::getMessage("IBLOCK_ADM_TO_EDIT"));

        /**
         * Здесь только столбы которые необходимо изменить
         */
        $row->AddCheckField("ACTIVE");
        $row->AddViewField("NAME", '<a href="corsik_yadelivery_zone_edit.php?ID=' . $f_ID . '&lang=' . LANGUAGE_ID . '">' . $f_NAME . '</a>');

        $arActions = [];
        $arActions[] = [
            "ICON" => "edit",
            "TEXT" => Loc::getMessage("MAIN_ADMIN_MENU_EDIT"),
            "ACTION" => $lAdmin->ActionRedirect("corsik_yadelivery_zone_edit.php?ID=" . $f_ID . "&lang=" . LANGUAGE_ID),
        ];
    ...
    $aContext = [
        [
            ...
            "LINK" => "corsik_yadelivery_zone_edit.php?lang=" . LANGUAGE_ID,
        ],
    ];
    ...
    $APPLICATION->SetTitle(GetMessage("CORSIK_DELIVERY_SERVICE_ZONES"));
