...
use Darneo\Ozon\Order\Fbo\Import as FboImport;

class OzonOrderFboListExchangeComponent extends CBitrixComponent
{
    private static array $moduleNames = ['darneo.ozon'];
    protected int $limit = 1000;
    protected int $page = 0;
    protected int $totalCount = 0;
    private FboImport $manager;
    ...
    private function setTemplateData(): void
    {
        ...
        $this->arResult['SIGNED_PARAMS'] = (new Signer())->sign(
            ...
            'darneo.ozon.order.fbo.list.exchange'
        );
    }
    ...
    {
        $filter = [
            'since' => (new DateTime())->add('-3 months')->format('Y-m-d\TH:i:s.u\Z'),
            'to' => (new DateTime())->format('Y-m-d\TH:i:s.u\Z')
        ];
        return $this->manager->initData($this->page, $this->limit, $filter);
    }
