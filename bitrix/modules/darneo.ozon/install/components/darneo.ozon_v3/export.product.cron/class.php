...
class OzonExportProductCronComponent extends CBitrixComponent
{
    ...
    private function getFieldHiddenValue($fieldName): string
    {
        ...
        return $signer->sign($params, 'darneo.ozon.export.product.cron');
    }
    ...
    {
        $row = $this->getList();
        $lang = Cron::getLang('EXPORT_CATALOG');
    ...
    {
        $settings = SettingsCronTable::getById('EXPORT_CATALOG')->fetch();
    ...
    private function update(): array
    {
        ...
        if ($result !== null) {
            if ($result->isSuccess()) {
                ...
                $status['MESSAGE'] = Loc::getMessage('DARNEO_OZON_MODULE_PRODUCT_CRON_UPDATE_SUCCESS');
            } else {
                $status['STATUS'] = 'ERROR';
            ...
            $status['STATUS'] = 'ERROR';
            $status['ERROR_LIST'] = [Loc::getMessage('DARNEO_OZON_MODULE_PRODUCT_CRON_UPDATE_ERROR')];
        }
