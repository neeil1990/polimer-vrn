...
class OzonExportPriceDetailComponent extends CBitrixComponent
{
    ...
    private function getFieldHiddenValue($fieldName): string
    {
        ...
        return $signer->sign($params, 'darneo.ozon.export.price.detail');
    }
    ...
    private function setTemplateData(): void
    {
        ...
        $this->arResult['SIGNED_PARAMS'] = (new Signer())->sign(
            ...
            'darneo.ozon.export.price.detail'
        );
    }
    ...
    private function update(): array
    {
            ...
            case 'FILTER':
                ...
                foreach ($params['FILTER_PROP'] as $filterRow) {
                    if (in_array($filterRow['ID'], $currentIds, true)) {
                        $result = PriceFilterTable::update(
                            $filterRow['ID'],
                            [
                                'ELEMENT_ID' => $this->elementId,
                ...
                foreach ($currentIds as $currentId) {
                    if (!in_array((int)$currentId, $newIds, true)) {
                        PriceFilterTable::delete($currentId);
                    }
                }
                ...
                return [
                    'STATUS' => 'SUCCESS',
                    'MESSAGE' => Loc::getMessage('DARNEO_OZON_MODULE_PRICE_DETAIL_UPDATE_SUCCESS'),
                ];
            default:
                if (!empty($params)) {
        ...
        if ($result !== null) {
            if ($result->isSuccess()) {
                ...
                $status['MESSAGE'] = Loc::getMessage('DARNEO_OZON_MODULE_PRICE_DETAIL_UPDATE_SUCCESS');
            } else {
                $status['STATUS'] = 'ERROR';
            ...
            $status['STATUS'] = 'ERROR';
            $status['ERROR_LIST'] = [Loc::getMessage('DARNEO_OZON_MODULE_PRICE_DETAIL_UPDATE_ERROR')];
        }
