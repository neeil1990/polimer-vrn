        ...
        <div v-if='isShowBlock'>
            <div class='card mb-5'>
                <div class='card-header'>
                    <h3 class='card-title'>
                        {{ $Bitrix.Loc.getMessage('DARNEO_OZON_VUE_PRODUCT_LIST_EXCHANGE_TITLE') }}
                    </h3>
                    <div class='card-toolbar'>
                        <span>
                            <span v-html='data.COUNT_HELPER'></span>
                            <span v-html='data.COUNT_CURRENT'></span>
                            {{ $Bitrix.Loc.getMessage('DARNEO_OZON_VUE_PRODUCT_LIST_EXCHANGE_FROM') }}
                ...
                <div class='card-body'>
                    ...
                    <div class='d-flex align-items-center'>
                        <button class='btn btn-primary mt-5' type='button' v-on:click='actionStart()'
                            ...
                            <span v-show='!isStart'>
                                {{ $Bitrix.Loc.getMessage('DARNEO_OZON_VUE_PRODUCT_LIST_EXCHANGE_BUTTON_START') }}
                            </span>
                            <span v-show='isStart'>
                                {{ $Bitrix.Loc.getMessage('DARNEO_OZON_VUE_PRODUCT_LIST_EXCHANGE_BUTTON_WORK') }}
                            </span>
                        ...
                        <span class='badge badge-warning text-dark ms-10 mt-5' v-show='!isStart && isFinished'>
                            {{ $Bitrix.Loc.getMessage('DARNEO_OZON_VUE_PRODUCT_LIST_EXCHANGE_COMPLETE') }}
                        </span>
                    </div>
                </div>
            </div>
        ...
        <div v-else class='d-flex flex-end align-items-center gap-2 gap-lg-3 mb-5'>
            <a href='javascript:void(0)' class='btn btn-sm btn-warning text-dark'
               v-on:click='isShowBlock=!isShowBlock'>
                {{ $Bitrix.Loc.getMessage('DARNEO_OZON_VUE_PRODUCT_LIST_EXCHANGE_BUTTON_SHOW') }}
            </a>
        </div>
        </div>
