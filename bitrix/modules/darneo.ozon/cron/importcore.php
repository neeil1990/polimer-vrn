...
const SETTING_CODE = 'IMPORT_CORE';
...
if (!$settings['VALUE'] || $settings['IS_STARTED']) {
    if ($settings['IS_STARTED'] && $settings['DATE_START'] instanceof Type\DateTime) {
        if ((new Type\DateTime()) > $settings['DATE_START']->add('180 minutes')) {
...
if ($update->isSuccess()) {
    (new Darneo\Ozon\Import\Core\Manager())->start();
}

SettingsCronTable::update(SETTING_CODE, ['DATE_FINISH' => new Type\DateTime(), 'IS_STARTED' => false, 'VALUE' => false]);
