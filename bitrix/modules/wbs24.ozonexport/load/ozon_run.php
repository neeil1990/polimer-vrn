...
if (!isset($wrappers)) $wrappers = new Wrappers();

// далее легаси код с вставками новых методов

$SETUP_VARS_LIST = 'IBLOCK_ID,SITE_ID,V,XML_DATA,SETUP_SERVER_NAME,COMPANY_NAME,SETUP_FILE_NAME,USE_HTTPS,FILTER_AVAILABLE,DISABLE_REFERERS,EXPORT_CHARSET,MAX_EXECUTION_TIME,SET_ID,SET_OFFER_ID,MIN_STOCK,IGNORE_SALE,BLOB,CONDITIONS,CHECK_PERMISSIONS';
$INTERNAL_VARS_LIST = 'intMaxSectionID,boolNeedRootSection,arSectionIDs,arAvailGroups';

// get settings - debug
if (false) {
	$vars = explode(",", $SETUP_VARS_LIST);
	$arr = [];
	foreach($vars as $name) {
		$arr[$name] = $$name;
	}
	\Wbs24\Ozonexport::showReturnAsCode($arr);
}
...
// загрузка данных из BLOB-параметров в XML_DATA
$XML_DATA = $ozon->loadBlobToExportSettings($XML_DATA, $BLOB);
...
$arOffers = $wrappers->CCatalogSku->GetInfoByProductIBlock($IBLOCK_ID);
...
if (empty($PROFILE_ID)) $PROFILE_ID = $profile_id ?? 0;
$BLOB['ignoreSale'] = $IGNORE_SALE;
$BLOB['minStock'] = $MIN_STOCK;
$BLOB['conditions'] = $CONDITIONS;
$BLOB['profileId'] = $PROFILE_ID;
$param = $ozon->getParams($BLOB);

// получение объекта формирования цен
$ozonPrice = $ozon->getPriceObject($param);

// получение объекта для вывода остатков по складам
$ozonWarehouse = $ozon->getWarehouseObject($param);

// получение объекта, который определяет, выводить ли позицию или нет
$ozonLimitations = $ozon->getLimitationObject($param);

// получение объекта, который проводит окончательную фильтрацию
$ozonFilter = $ozon->getFilterObject($param);

// получение объекта журнала остатков
...
$db_iblock = $wrappers->CIBlock->GetByID($IBLOCK_ID);
...
else
{
	...
	if (!empty($arOffers['IBLOCK_ID']))
	{
		...
		$rsOfferIBlocks = $wrappers->CIBlock->GetByID($intOfferIBlockID);
...
if ($firstStep)
{
    $ozonOffersLog->startOffersLog();
    $ozonOffersLog->clearOldOffersLog();
...
if (empty($arRunErrors))
{
	...
	do
	{
		...
		$iterator = $wrappers->CIBlockElement->GetList(
			array('ID' => 'ASC'),
			$filter,
			false,
			$navParams,
		...
		if (!empty($items))
		{
			...
			if ($needProperties || $needDiscountCache)
			{
				...
				{
					$wrappers->CIBlockElement->GetPropertyValuesArray(
						$items,
						$IBLOCK_ID,
						array(
							'ID' => $itemIdsList,
			...
			foreach ($itemIdsList as $id)
			{
				...
				if ($row['TYPE'] == Catalog\ProductTable::TYPE_SKU && !empty($row['OFFERS']))
				{
					...
					foreach ($row['OFFERS'] as $offer)
					{
                        ...
                        $premiumPrice = $ozonPrice->getPremiumPrice($price, $fullPrice);
						$allowShow = $ozonLimitations->verifyElementShowing($offer, [
							'minPrice' => $minPrice,
							'fullPrice' => $fullPrice,
							'price' => $price,
							'oldPrice' => $oldPrice,
                        ...
                        if ($allowShow) {
                            ...
                            $allowShow = $ozonFilter->verifyElementShowing($offer);
                        }

						if (!$allowShow) continue;

                        $ozonOffersLog->addOfferToLog([
                            'offer_id' => $offerId,
                            'price' => $price,
                        ]);
                        ...
                        // wbs24 min_price
                        if ($newMinPrice) $itemsContent .= "<min_price>".$newMinPrice."</min_price>\n";

                        // wbs24 premium цена
                        if ($premiumPrice !== false) $itemsContent .= "<premium_price>".$premiumPrice."</premium_price>\n";

						// wbs24 количество (остаток)
						$itemsContent .= $ozonWarehouse->getXml($offer);
				...
				elseif (isset($simpleIdsList[$id]) && !empty($row['PRICES']))
				{
                    ...
                    $newMinPrice = $ozonPrice->getMinPrice($price, $fullPrice);
                    $premiumPrice = $ozonPrice->getPremiumPrice($price, $fullPrice);
					$allowShow = $ozonLimitations->verifyElementShowing($row, [
						'minPrice' => $minPrice,
						'fullPrice' => $fullPrice,
						'price' => $price,
						'oldPrice' => $oldPrice,
					]);
                    if ($allowShow) $allowShow = $ozonFilter->verifyElementShowing($row);

					if ($price <= 0 || !$allowShow) continue;
					...
					// wbs24 количество (остаток)
					$itemsContent .= $ozonWarehouse->getXml($row);
	...
	if ($finalExport)
	{
        ...
        */
		...
		if ($process)
		{
            ...
            $nullStocksXml = $ozonWarehouse->getXml([]);
            $content .= $ozonOffersLog->getNullOffersAsXml($nullStocksXml);
		...
		if ($process)
		{
            ...
            if (!$ozon->updateExportFile($SETUP_FILE_NAME, $sectionFileName))
			{
				$arRunErrors[] = str_replace('#FILE#', $sectionFileName, GetMessage('BX_CATALOG_EXPORT_YANDEX_ERR_UPDATE_LINK_TO_FILE'));
			}
		}
		unset($process);
	}
...
if (!empty($arRunErrors)) {
    $ozon->errorHandler($arRunErrors, $PROFILE_ID);
