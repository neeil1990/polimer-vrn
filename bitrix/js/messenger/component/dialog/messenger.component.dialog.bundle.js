	...
	BX.Vue.component('bx-messenger-dialog', {
	  ...
	  computed: {
	    ...
	    dialog: function dialog() {
	      var dialog = this.$store.state[MessengerStore.dialogues].collection[this.dialogId];
	      return dialog ? dialog : {};
	    },
	    collection: function collection() {
	      var collection = this.$store.state[MessengerStore.messages].collection[this.chatId];
	  ...
	  methods: {
	    onDialogClick: function onDialogClick(event) {
	      if (BX.Vue.testNode(event.target, {
	    ...
	    onRequestUnreadAnswer: function onRequestUnreadAnswer() {
	      ...
	      if (event.count > 0) {
	        ...
	        var _body = this.$refs.body;
	        _body.scrollTop = _body.scrollHeight - _body.offsetHeight;
	        this.waitLoadUnread = false;
	      }
...
//# sourceMappingURL=messenger.component.dialog.bundle.js.map
